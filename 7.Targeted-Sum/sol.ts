 ...
 */
function findTarget_HashMap(nums: number[], target: number): number[] | string {
...
// ======================================================================
// <PRACTICE_START>
function findTarget(nums: number[], target: number): number[] | string {
...
function runTests() {
  ...
  const fmt = (val: any) => (Array.isArray(val) ? `[${val.join(", ")}]` : val);
